-- Mission Script Retriever by By: hejoro
...
if not JSON then
	   ...
	   while i <= text_len do
		  ...
		  elseif text:match('^\\t', i) then
			 ...
			 if hex then
				...
				-- followed by another in a specific range, it will be a two-code surrogate pair.
				local codepoint = tonumber(hex, 16)
				if codepoint >= 0xD800 and codepoint <= 0xDBFF then
				   -- it is a hi surrogate... see whether we have a following low
				   ...
				   if lo_surrogate then
					  ...
					  -- not a proper low, so we will just leave the first codepoint as is and spit it out.
				   end
				end
				...
				-- just pass through what has escaped
	...
	local function grok_array(self, text, start, etc)
	   ...
	   while i <= text_len do
		  ...
		  -- can not table.insert(VALUE, val) here because it is a no-op if val is nil
	...
	function OBJDEF:decode(text, etc)
	   ...
	   if text:match('^%s*<') then
		  -- Cannot be JSON... we will assume it is HTML
		  self:onDecodeOfHTMLError(string.format("html passed to JSON:decode()"), text, nil, etc)
	   end

	   --
	   -- Ensure that it is not UTF-32 or UTF-16.
	   -- Those are perfectly valid encodings for JSON (as per RFC 4627 section 3),
	   -- but this package cannot handle them.
	   --
	...
	local function object_or_array(self, T, etc)
	   --
	   -- We need to inspect all the keys... if there are any strings, we will convert to a JSON
	   -- object. If there are only numbers, it is a JSON array.
	   --
	   -- If we will be converting to a JSON object, we will want to sort the keys so that the
	   ...
	   if #string_keys == 0 and not number_keys_must_be_strings then
		  ...
		  elseif tostring(T) == "JSON object" then
			 ...
			 -- have to guess, so we will pick array, since empty arrays are likely more common than empty objects
			 return nil
		  end
	   ...
	   if #number_keys > 0 then
		  ...
		  -- If we are here then we have either mixed string/number keys, or numbers inappropriate for a JSON array
		  -- It is not ideal, but we will turn the numbers into strings so that we can at least create a JSON object.
		  --
	...
	function encode_value(self, value, parents, etc, indent) -- non-nil indent means pretty-printing
	   ...
	   elseif type(value) == 'number' then
		  ...
		  elseif value >= math.huge then
			 ...
			 -- is both ">= math.huge" and "<= -math.huge", which makes no sense but that is how it is.
       ...
       elseif type(value) == 'userdata' then       --<< inserted
          return json_string_literal(tostring(value)) --<< inserted
	   ...
	   elseif type(value) ~= 'table' then
		  ...
		  elseif object_keys then
			 ...
			 if indent then
				...
				local subtable_indent = key_indent--indent .. string.rep(" ", max_key_length + 2 + 4)
			 ...
			 -- An empty array/object... we will treat it as an array, though it should really be an option
			 --
...
function MissionManager:parse(params, stage_name, offset, file_type)
	...
	-- "stage_name" = stage1
	self.Retriever = params
	return tMSR_MM_Parse(self, params, stage_name, offset, file_type)
end

function MissionManager:Retriever()
	local script = self:_serialize_to_script( "mission", self.Retriever.file_path )
	local levelID= managers.job:current_level_id()
	
	SystemFS:make_dir("mission_scripts/" .. levelID)
	
	local  file = io.open("mission_scripts/".. levelID .."/_all.json", "w")
	...
	for k,v in pairs(script) do
		local file = io.open("mission_scripts/".. levelID .."/"..k..".json", "w")
		file:write(JSON:encode_pretty(v))
		file:close()
	end
